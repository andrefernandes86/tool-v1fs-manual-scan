...
- **Scan**: Live progress with **Target(s)** (the paths being scanned), elapsed time, files scanned, malicious count, and current file; banner listing detected malware; PDF report at the end. On Linux, **`/proc`**, **`/sys`**, **`/dev`**, and **`/run`** are skipped (including typical bind-mount paths like `.../sys/...`). Permission errors on those trees are treated as expected and do not inflate the scan error count.
